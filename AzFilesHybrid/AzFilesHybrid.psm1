...
function Get-ServicePrincipalName {
    ...
    param (
        ...
        [Parameter(Mandatory=$false, Position=2, HelpMessage="Service class name")]
        [string]$ServiceClassName = "cifs"
    )

    $fileEndpoint = Get-AzStorageAccountFileEndpoint -ResourceGroupName $ResourceGroupName `
        -StorageAccountName $StorageAccountName -ErrorAction Stop

    $servicePrincipalName = $fileEndpoint -replace 'https://', ($ServiceClassName + '/')
...
function Set-StorageAccountAadKerberosADProperties {
    <#
    .SYNOPSIS
        This sets the storage account's ActiveDirectoryProperties - infomration needed to support the shell
        experience for getting and setting file and directory permissions.

        Because AzureAD does not support configuring ACLs in Shell, for Public Preview, clients 
        must instead rely on their Active Directory.
...
function Update-AzStorageAccountAzureADKerberosPassword {
    ...
    process {
        ...
        if ($StorageAccount.AzureFilesIdentityBasedAuth.DirectoryServiceOptions -ne "AADKERB") {
            Write-Error `
                -Message ("Storage account " + $StorageAccount.StorageAccountName + " has not configured for Azure AD Kerberos.") `
                -ErrorAction Stop
        }
...
function Enable-ApiPermissionsForAzureADApplication
{
	param(
		[Parameter(Mandatory=$true)]
	...
	process {
        ...
        Write-Warning "Interactive consent required to complete setup. Navigate to $($url) to enable admin consent for $($ChildApp.DisplayName)";
...
function Set-StorageAccountAadKerbDirectoryService {
    ...
    try {
        ...
        Write-Host $_.Exception.ToString()
        Write-Host "Error setting Storage Account directoryServiceOptions=AADKERB.  StatusCode:" $_.Exception.Response.StatusCode.value__ 
        Write-Host "Error setting Storage Account directoryServiceOptions=AADKERB.  StatusDescription:" $_.Exception.Response.StatusDescription
        Write-Error -Message "Caught exception setting Storage Account directoryServiceOptions=AADKERB: $_" -ErrorAction Stop
    }
}

function Join-AzStorageAccountForAzureADKerberos {
    ...
    process {
        ...
        Set-StorageAccountAadKerbDirectoryService `
             -ResourceGroupName $ResourceGroupName -StorageAccountName $StorageAccountName -ErrorAction Stop;
        ...
        $servicePrincipalNames[0] = Get-ServicePrincipalName -StorageAccountName $StorageAccountName -ResourceGroupName $ResourceGroupName -ServiceClassName 'HTTP';
        $servicePrincipalNames[1] = Get-ServicePrincipalName -StorageAccountName $StorageAccountName -ResourceGroupName $ResourceGroupName -ServiceClassName 'CIFS';
        $servicePrincipalNames[2] = Get-ServicePrincipalName -StorageAccountName $StorageAccountName -ResourceGroupName $ResourceGroupName -ServiceClassName 'HOST';
        ...
        Update-AzStorageAccountAzureADKerberosPassword `
            -RotateToKerbKey "kerb1" -ResourceGroupName $ResourceGroupName -StorageAccountName $StorageAccountName `
            -SkipShouldProcess -ErrorAction Stop;

        <# Grant the required API permissions to the application object. #>
		Enable-ApiPermissionsForAzureADApplication -targetServicePrincipalName 'Microsoft Graph' `
            -appPermissionsRequired @('openid','User.Read','profile') -childApp $application -ServicePrincipalForApp $servicePrincipal -ErrorAction Stop
    }
}
